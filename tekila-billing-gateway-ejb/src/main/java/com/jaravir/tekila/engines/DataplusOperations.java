 ...
 */
@Stateless(name = "DataplusOperations", mappedName = "DataplusOperations")
@TransactionManagement(TransactionManagementType.CONTAINER)
@TransactionAttribute(TransactionAttributeType.REQUIRED)
public class DataplusOperations implements OperationsEngine {
    private final static Logger log = LoggerFactory.getLogger(DataplusOperations.class);
    ...
    public Subscription createSubscriptionSpring(SubscriptionCreationVM subscriptionCreationVM, Subscriber selectedSubscriber, Subscription subscription, Service service, User user) {
        ...
        log.info("DataPlus createSubscriptionSpring method starts..");
    ...
    public void createSubscription(Subscriber selectedSubscriber, Subscription subscription, String serviceId, MiniPop miniPop, boolean isUseStockEquipment, Equipment equipment, List<NotificationSettingRow> notificationSettings, double installationFeeRate, Campaign selectedCampaign, HttpSession session, User user, String... additionalProperties) throws NoFreePortLeftException, PortAlreadyReservedException {
        ...
        if (subscription.getService().getServiceType() == ServiceType.BROADBAND) {
            ...
            if (additionalProperties != null &&
                    ...
                    !additionalProperties[1].isEmpty()) { //serviceType
                serviceType = serviceTypePersistenceFacade.find(Long.parseLong(additionalProperties[1]));
            }

            String username = null;
            if (additionalProperties != null &&
                    additionalProperties.length > 3 &&
                    additionalProperties[3] != null &&
                    !additionalProperties[3].isEmpty()){
                username = additionalProperties[3].toString();
            }
            ...
            if (additionalProperties != null &&
                    ...
                    !additionalProperties[0].isEmpty()) { //zoneId
                subscriptionSetting = new SubscriptionSetting();
                subscriptionSetting.setValue(additionalProperties[0]);
                serviceSetting = commonOperationsEngine.createServiceSetting("ZONE", ServiceSettingType.ZONE, Providers.DATAPLUS, ServiceType.BROADBAND, "");
            ...
            if (additionalProperties != null &&
                ...
                serviceSetting = commonOperationsEngine.createServiceSetting("SERVICE_TYPE", ServiceSettingType.SERVICE_TYPE, Providers.DATAPLUS, ServiceType.BROADBAND, "");
                subscriptionSetting.setProperties(serviceSetting);
                settingFacade.save(subscriptionSetting);
                subscriptionSetting = settingFacade.update(subscriptionSetting);
                settingList.add(subscriptionSetting);
            ...
            if (additionalProperties != null &&
                    additionalProperties.length > 2 &&
                    additionalProperties[2] != null &&
                    !additionalProperties[2].isEmpty()) { //dealer
                subscriptionSetting = new SubscriptionSetting();
                subscriptionSetting.setValue(additionalProperties[2]);
                serviceSetting = commonOperationsEngine.createServiceSetting("DEALER", ServiceSettingType.DEALER, Providers.DATAPLUS, ServiceType.BROADBAND, "");
                subscriptionSetting.setProperties(serviceSetting);
                settingFacade.save(subscriptionSetting);
                subscriptionSetting = settingFacade.update(subscriptionSetting);
                settingList.add(subscriptionSetting);
            ...
            if (serviceType != null && serviceType.getProfile().getProfileType().equals(ProfileType.IPoE)) {
                subscriptionSetting = new SubscriptionSetting();
                ...
                Port port = miniPopPersistenceFacade.getAvailablePort(miniPop);
                ...
                subscriptionSetting = new SubscriptionSetting();
                subscriptionSetting.setValue(String.format("Ethernet0/0/%d:%s", port.getNumber(), miniPop.getMac()));
                serviceSetting = commonOperationsEngine.createServiceSetting("USERNAME", ServiceSettingType.USERNAME, Providers.DATAPLUS, ServiceType.BROADBAND, "");
                subscriptionSetting.setProperties(serviceSetting);
                settingFacade.save(subscriptionSetting);
                subscriptionSetting = settingFacade.update(subscriptionSetting);
                settingList.add(subscriptionSetting);
                ...
                subscriptionSetting = new SubscriptionSetting();
                subscriptionSetting.setValue("cisco");
                ...
                subscriptionSetting = new SubscriptionSetting();
                subscriptionSetting.setValue(username);
                serviceSetting = commonOperationsEngine.createServiceSetting("USERNAME", ServiceSettingType.USERNAME, Providers.DATAPLUS, ServiceType.BROADBAND, "");
                subscriptionSetting.setProperties(serviceSetting);
                settingFacade.save(subscriptionSetting);
                subscriptionSetting = settingFacade.update(subscriptionSetting);
            ...
            log.debug("ZONE: " + subscription.getSettingByType(ServiceSettingType.ZONE));
        ...
        if (selectedSubscriber.getLifeCycle() == SubscriberLifeCycleType.PREPAID) {
            ...
            if (rate == null) {
                ...
                if (prop == null) {throw new RuntimeException("Could not find monthly price amount");}
                rate = prop.getPrice();
            }
...
//            subscription.setUser(user);
//            subscription.setSubscriber(selectedSubscriber);
//            subscription.setService(serviceFacade.find(serviceId));
//            subscription.setStatus(SubscriptionStatus.INITIAL);
//
//            log.debug("the created subscription: " + subscription);
//
...
//    }
    ...
    public void activatePrepaid(Subscription subscription) {
        subscription = subscriptionPersistenceFacade.update(subscription);
        log.debug("ActivatePrepaid for subscription " + subscription.getId());
