...
@Stateless(name = "CNCPaymentProcessor", mappedName = "CNCPaymentProcessor")
public class CNCPaymentProcessor implements PaymentProcessorEngine {
    @EJB
    ...
    public void process(Subscription subscription, long amount) {
        log.debug("==== CNCPaymentProcessor for subscription " + subscription.getId());
...
//            e.printStackTrace();
            log.error("CNC Provisioner not found when process payment, subscription id: "+subscription.getId(), e);
        }
        ...
        if ((subscription.getStatus() == SubscriptionStatus.INITIAL
            ...
            if (subscription.getSubscriber().getLifeCycle() == SubscriberLifeCycleType.PREPAID
                ...
                log.info(String.format("subscritiption id=%s, status=%s activating", subscription.getId(), subscription.getStatus()));
