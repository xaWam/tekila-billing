...
@Stateless(name = "UninetPaymentProcessor", mappedName = "UninetPaymentProcessor")
public class UninetPaymentProcessor implements PaymentProcessorEngine {
    ...
    private final static Logger log = LoggerFactory.getLogger(UninetPaymentProcessor.class);
    ...
    public void process(Subscription subscription, long amount) {
        ...
        log.debug("==== UninetPaymentProcessor for subscription " + subscription.getId());
        ...
        try {
            ...
            e.printStackTrace();
            log.error("Uninet Provisioner not found when process payment, subscription id: "+subscription.getId(), e);
        }
        ...
        if (subscription.getSubscriber().getLifeCycle() == SubscriberLifeCycleType.PREPAID
            ...
            log.debug("Prolongprepaid starts for subscription {}", subscription.getId());
            ...
            log.debug("Processing of onPayment for Uninet Subscription {}", subscription.getId());
            ...
            if (subscription.getStatus() == SubscriptionStatus.ACTIVE) {
                log.debug("Subscription's [{}] status is ACTIVE. Skipping ... ", subscription.getId());
                return;
            }
            ...
            if (subscription.getActivationDate() == null) {
                ...
                if (subscription.getActivationDate() == null && fd == null) {
                    log.debug("Subscription {} is not Activated on Radius [fd={}]. Skipping...", subscription.getId(), fd);
                    return;
                }
            ...
            log.debug("Uninet Payment Processor -> "+subscription);
            ...
            if (subscription.getStatus() == SubscriptionStatus.INITIAL) {
                ...
                if (subscription.getBalance().getRealBalance() >= 0) {
                    ...
                    if (provisioner.openService(subscription, subscription.getExpirationDateWithGracePeriod())) {
                        log.debug("subscription id={} switched from initial to active, prolonged on radius db", subscription.getId());
                        systemLogger.success(
                                SystemEvent.SUBSCRIPTION_PROLONGED,
                                subscription,
                                String.format("subscription id=%d switched from initial to active, prolonged on radius db", subscription.getId()));
            ...
            if (subscription.getBillingModel().getPrinciple() == BillingPrinciple.CONTINUOUS) {
                ...
                if (subscription.getStatus() == SubscriptionStatus.BLOCKED
                    ...
                    log.debug("CONTINUOUS. Status is BLOCKED and Balance is >= 0 for subscription {}", subscription.getId());
                ...
                log.debug("GRACE. Status is BLOCKED and Balance is >= 0 for subscription {}", subscription.getId());
                ...
                if (provisioner.openService(subscription, subscription.getExpirationDateWithGracePeriod())) {
                    ...
                    && subscription.getBalance().getRealBalance() >= 0) {
                log.debug("GRACE. Status is PARTIALLY_BLOCKED and Balance is >= 0 for subscription {}", subscription.getId());
