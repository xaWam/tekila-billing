 ...
 */
@Stateless(name = "UninetOperations", mappedName = "UninetOperations")
@TransactionManagement(TransactionManagementType.CONTAINER)
@TransactionAttribute(TransactionAttributeType.REQUIRED)
public class UninetOperations implements OperationsEngine {
    private final static Logger log = LoggerFactory.getLogger(UninetOperations.class);
    ...
    public Subscription createSubscriptionSpring(SubscriptionCreationVM subscriptionCreationVM,
        ...
        log.info("Uninet createSubscriptionSpring method starts..");
...
//        }
        ...
        try {
            ...
            /*
            Integer bm = subscriptionCreationVM.getBillingModel();
            BillingPrinciple billingPrinciple = bm == 1 ? BillingPrinciple.GRACE_MONTH:
                                                bm == 2 ? BillingPrinciple.GRACE:
                                                bm == 3 ? BillingPrinciple.CONTINUOUS:
                                                BillingPrinciple.CONTINUOUS;
             */

            BillingPrinciple billingPrinciple = subscriptionCreationVM.getBillingPrinciple();
            BillingModel billingModel = modelFacade.find(billingPrinciple);
            log.debug("Billing model: " + billingModel);
            subscription.setBillingModel(billingModel);

            subscription.getDetails().setLanguage(Language.AZERI); //hu hisse deyishilmelidi
...
//        }
    ...
    public void createSubscription(Subscriber selectedSubscriber,
        ...
        if (subscription.getService().getServiceType() == ServiceType.BROADBAND) {
            ...
            ServiceSetting serviceSetting = commonEngine.createServiceSetting("PASSWORD", ServiceSettingType.PASSWORD, Providers.UNINET, ServiceType.BROADBAND, "");
            ...
            if (additionalProperties != null &&
                    ...
                    !additionalProperties[0].isEmpty()) {
                subscriptionSetting = new SubscriptionSetting();
                subscriptionSetting.setValue(additionalProperties[0]);
                serviceSetting = commonEngine.createServiceSetting("DEALER", ServiceSettingType.DEALER, Providers.UNINET, ServiceType.BROADBAND, "");
                subscriptionSetting.setProperties(serviceSetting);
                settingFacade.save(subscriptionSetting);
                subscriptionSetting = settingFacade.update(subscriptionSetting);
                settingList.add(subscriptionSetting);
    ...
    private Subscription addVASWithSetting(
        ...
        try {
            ...
            ServiceSetting serviceSetting = commonEngine.createServiceSetting("IP_ADDRESS", ServiceSettingType.IP_ADDRESS, Providers.UNINET, ServiceType.BROADBAND, "");
            subscriptionSetting.setProperties(serviceSetting);
            settingFacade.save(subscriptionSetting);
            subscriptionSetting = settingFacade.update(subscriptionSetting);
    ...
    public Subscription removeVAS(Subscription subscription, SubscriptionVAS vas) {
        try {
            ...
            if (subscription.getSettingByType(ServiceSettingType.IP_ADDRESS) != null) {
                ...
                log.info("removeVAS: ipAddressAsString="+ipAddressAsString+", ipAddress="+ipAddress+", subscription id="+subscription.getId()); //ipAddress can be null, so exception will throw in the next row
